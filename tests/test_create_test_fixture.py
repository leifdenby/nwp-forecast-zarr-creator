...
def test_dest_prefix_includes_analysis_time():
    assert (
        cf.dest_prefix("my-bucket", _utc(2025, 3, 2, 6))
        == "s3://my-bucket/2025-03-02T0600Z"
    )
    assert (
        cf.dest_prefix("memory://my-bucket", _utc(2025, 3, 2, 6))
        == "memory://my-bucket/2025-03-02T0600Z"
    )
...
def test_create_dry_run_uploads_nothing():
    ...
    assert prefix == "memory://cf-dry-dst/2025-03-02T0600Z"
